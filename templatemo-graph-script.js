...
form.addEventListener("submit", function (e) {
    ...
    // Only run success part if all valid
    if (isValid) {
        const formContainer = document.querySelector(".contact-form");
        formContainer.classList.add("fade-out");

        // Wait for fade out to finish then show message
        setTimeout(() => {
            formContainer.innerHTML = `
                <h3 class="fade-in" style="color: green; text-align: center; font-size: 24px;">
                    Message sent successfully!
                </h3>
            `;
        }, 500); // 0.5s = same as fade-out duration
    }
});
